...
def configure_sc_dynamics(self, initial_conditions):
        ...
        oe = initial_conditions.get("oe")
        rN = initial_conditions.get("rN")
        vN = initial_conditions.get("vN")

        width = initial_conditions.get("width")
        depth = initial_conditions.get("depth")
        height = initial_conditions.get("height")
        ...
        sigma_init = initial_conditions.get("sigma_init")
        omega_init = initial_conditions.get("omega_init")

        scObject.hub.sigma_BNInit = sigma_init  # sigma_BN_B
        scObject.hub.omega_BN_BInit = omega_init

        dragEffector = facetDragDynamicEffector.FacetDragDynamicEffector()
        #   Set up the goemetry of a 6U cubesat
        dragEffector.addFacet(0.2*0.3, 2.2, [1,0,0], [0.05, 0.0, 0])
        dragEffector.addFacet(0.2*0.3, 2.2, [-1,0,0], [0.05, 0.0, 0])
        dragEffector.addFacet(0.1*0.2, 2.2, [0,1,0], [0, 0.15, 0])
        dragEffector.addFacet(0.1*0.2, 2.2, [0,-1,0], [0, -0.15, 0])
        dragEffector.addFacet(0.1*0.3, 2.2, [0,0,1], [0,0, 0.1])
        dragEffector.addFacet(0.1*0.3, 2.2, [0,0,-1], [0, 0, -0.1])
        dragEffector.addFacet(1.*2., 2.2, [0,1,0],[0,2.,0])
        dragEffector.addFacet(1.*2., 2.2, [0,-1,0],[0,2.,0])

        dragEffector.atmoDensInMsgName = densityModel.envOutMsgNames[-1]
        scObject.addDynamicEffector(dragEffector)

        #   Disturbance Torque Setup
        disturbance_magnitude = initial_conditions.get("disturbance_magnitude")
        disturbance_vector = initial_conditions.get("disturbance_vector")
        unit_disturbance = disturbance_vector/np.linalg.norm(disturbance_vector)
        extForceTorqueObject = extForceTorque.ExtForceTorque()
        extForceTorqueObject.extTorquePntB_B = disturbance_magnitude * disturbance_vector
        extForceTorqueObject.ModelTag = 'DisturbanceTorque'
        extForceTorqueObject.cmdForceInertialInMsgName = 'disturbance_torque'
        scObject.addDynamicEffector(extForceTorqueObject)

        # Add reaction wheels to the spacecraft
        rwStateEffector, rwFactory, initWheelSpeeds = ap.balancedHR16Triad(useRandom=True,randomBounds=(-800,800))
        # Change the wheel speeds
        rwFactory.rwList["RW1"].Omega = initial_conditions.get("wheelSpeeds")[0]*mc.RPM #rad/s
        rwFactory.rwList["RW2"].Omega = initial_conditions.get("wheelSpeeds")[1]*mc.RPM #rad/s
        rwFactory.rwList["RW3"].Omega = initial_conditions.get("wheelSpeeds")[2]*mc.RPM #rad/s
        initWheelSpeeds = initial_conditions.get("wheelSpeeds")
        rwStateEffector.InputCmds = "rwTorqueCommand"
        rwFactory.addToSpacecraft("ReactionWheels", rwStateEffector, scObject)
        rwConfigMsgName = "rwConfig"
        unitTestSupport.setMessage(TotalSim, DynamicsProcessName, rwConfigMsgName, rwFactory.getConfigMessage(), msgStrName="RWArrayConfigFswMsg")

        #   Add thrusters to the spacecraft
        thrusterSet, thrFactory = ap.idealMonarc1Octet()
        thrusterSet.InputCmds = "rwDesatTimeOnCmd"
        thrModelTag = "ACSThrusterDynamics"
        thrusterConfigMsgName = "thrusterConfig"
        unitTestSupport.setMessage(TotalSim, DynamicsProcessName, thrusterConfigMsgName, thrFactory.getConfigMessage(), msgStrName="THRArrayConfigFswMsg")
        thrFactory.addToSpacecraft(thrModelTag, thrusterSet, scObject)
        ...
        solarPanel.ModelTag = 'solarPanel' + str(sc_number)
        solarPanel.stateInMsgName = scObject.scStateOutMsgName
        solarPanel.sunEclipseInMsgName = 'eclipse_data_'+str(sc_number)
        solarPanel.sunInMsgName = 'sun_planet_data'
        solarPanel.setPanelParameters(unitTestSupport.np2EigenVectorXd(initial_conditions.get("nHat_B")), initial_conditions.get("panelArea"), initial_conditions.get("panelEfficiency"))
        solarPanel.nodePowerOutMsgName = "panelPowerMsg" + str(sc_number)

        powerSink = simplePowerSink.SimplePowerSink()
        powerSink.ModelTag = "powerSink" + str(sc_number)
        powerSink.nodePowerOut = powerDraw  # Watts
        powerSink.nodePowerOutMsgName = "powerSinkMsg" + str(sc_number)
        ...
        powerMonitor.storageCapacity = initial_conditions.get("storageCapacity")
        powerMonitor.storedCharge_Init = initial_conditions.get("storedCharge_Init")
...
class CoordinatedGroundObsSimulator(SimulationBaseClass.SimBaseClass):
    '''
    Simulates a pair of spacecraft in LEO with atmospheric drag and J2.
    ...
    def set_dynamics(self):
        ...
        self.gravFactory.createSpiceInterface(bskPath + '/supportData/EphemerisData/'
                                              , timeInitString
                                              , spicePlanetNames=["sun", "earth", "moon"]
                                              )
        ...
        for ind, initial_condition in self.initial_conditions:
            configure_sc_dynamics(initial_condition, ind)
        ...
        #   Add the environment dynamics to the environment task
